...
void setup_timers(event_loop_t &event_loop) {
    // create twenty timers

    for (int i = 0; i < NUM_TIMERS; i++) {
        timer_ticks[i] = 0;
        int delay = (i + 1) * (i + 1);
        event_loop.onRepeat(delay, [i]() { timer_ticks[i]++; });
    }
...
void setup_tick(event_loop_t &event_loop) {
    // increase the tick counter on every tick
    event_loop.onTick([]() { tick_counter++; });
}
